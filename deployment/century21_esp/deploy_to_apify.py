"""
Script pour créer et déployer l'actor Century21 Spain sur Apify via l'API REST
"""
import requests
import os
...
ACTOR_NAME = "century21-spain-scraper"
ACTOR_TITLE = "Century21 Spain Scraper"
ACTOR_DESCRIPTION = "Scrape tous les agents Century21 Spain et push dans Supabase staging"
...
def get_or_create_actor():
    ...
    response = requests.post(f"{BASE_URL}/acts", json=actor_data, headers=headers)
    ...
    for file_path, target_name in files_to_include:
        ...
        if full_path.exists():
            with open(full_path, 'r', encoding='utf-8') as f:
                content = f.read()
            files_array.append({"name": target_name, "content": content})
            print(f"  + {file_path}")
        else:
...
def main():
    print("=" * 50)
    print("Deploiement Century21 Spain Scraper sur Apify")
    print("=" * 50)
