"""
Actor Apify pour scraper les agents ExP Spain
1. Récupère tous les agents via l'API Supabase ExP (pagination)
2. Enrichit avec les données de sites web (LinkedIn) et listings (mandats, prix, ville)
...
NETWORK_NAME = "ExP Spain"


#### API CALL FUNCTIONS ####

def fetch_agents_page(offset: int, limit: int, api_key: str):
    """Fetches a paginated batch of active Spanish agents."""
    headers = {
        "apikey": api_key,
...
def parse_agent_data(response):
    ...
    agent["arrival_date"] = None  # Not available in Spain's CRM
...
async def main():
    ...
    async with Actor:
        ...
        Actor.log.info("=" * 50)
        Actor.log.info("=== ExP Spain Scraper ===")
